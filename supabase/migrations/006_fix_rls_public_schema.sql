...
-- This uses a SECURITY DEFINER function in the PUBLIC schema to break the circular reference

-- First, drop all existing policies
DROP POLICY IF EXISTS "Users can view own and partner profile" ON users;
DROP POLICY IF EXISTS "Users can view own profile" ON users;
DROP POLICY IF EXISTS "Users can view partner profile" ON users;
...
-- Create a SECURITY DEFINER function in PUBLIC schema to get user's couple_id without triggering RLS
...
CREATE POLICY "Users can view their item photos"
  ...
  USING (
    EXISTS (
      ...
      AND items.couple_id = public.get_my_couple_id()
    )
  );
